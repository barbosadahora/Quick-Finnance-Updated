...
public class GoalService {
    ...
    public Goal updateGoal(Long id, Goal goalDetails, User user) {
        ...
        if (optionalGoal.isPresent()) {
            ...
            goal.setAlertThreshold(goalDetails.getAlertThreshold());
    ...
    public Goal addToGoal(Long id, Double amount, User user) {
        ...
        if (optionalGoal.isPresent()) {
            ...
            try {
                ...
                if (goal.getAlertThreshold() != null && goal.getCurrentAmount() >= (goal.getTargetAmount() * goal.getAlertThreshold() / 100)) {
                    String message = String.format("Sua meta \'%s\' atingiu %.2f%% do valor alvo!", goal.getName(), goal.getAlertThreshold());
                    notificationService.createNotification(user, message, "GOAL_ALERT");
                }
                return goalRepository.save(goal);
