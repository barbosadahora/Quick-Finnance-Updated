...
export function Transactions() {
  ...
  }, [isAuthenticated, searchTerm, startDate, endDate, categoryFilter, filter]);
  ...
  const loadTransactions = async () => {
    try {
      ...
      if (searchTerm) {
        data = await transactionsService.searchTransactions(searchTerm);
      } else if (startDate && endDate) {
        data = await transactionsService.filterTransactionsByDateRange(startDate, endDate);
      } else if (categoryFilter) {
        data = await transactionsService.filterTransactionsByCategory(categoryFilter);
      } else if (filter !== 'all') {
        data = await transactionsService.filterTransactionsByType(filter);
      } else {
        data = await transactionsService.getTransactions();
      }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div>
            <Label htmlFor="searchTerm">Buscar por Descrição</Label>
            <Input type="text" id="searchTerm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar..." />
          </div>
          <div>
